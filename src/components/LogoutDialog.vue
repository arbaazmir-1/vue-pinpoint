...
div(class="z-10 w-screen h-screen dark:bg-[#00000070] bg-[#f9f9f990] flex items-center justify-center fixed inset-0") 
    div(class="animate-fade-in z-20 md:w-3/5 w-11/12 lg:w-2/5 xl:w-1/5 h-fit dark:bg-gray-800 bg-white shadow-lg flex-col rounded-md p-5 space-y-3")
        ...
        p(class="text-sm") You will have to login with email and password to use pinpoint
        div(class="body-dialog flex flex-col w-full space-y-4")
            
            div(class="flex w-full justify-end space-x-2")
                Button(@click="emit('closeDialog')") Cancel
                Button(@click="emit('logoutConfirmed')" variant="destructive") Delete
</template>

<script setup>
import { Button } from './ui/button'

const emit = defineEmits(['closeDialog', 'logoutConfirmed'])
</script>
